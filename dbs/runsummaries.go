...
func (a *API) RunSummaries() error {
	...
	if err != nil {
		return Error(err, ParseErrorCode, "unable to parse run_num value", "dbs.runsummaries.RunSummaries")
	}
	...
	if len(dataset) == 1 {
		if strings.Contains(dataset[0], "*") {
			...
			return Error(InvalidParamErr, InvalidParameterErrorCode, msg, "dbs.runsummaries.RunSummaries")
		}
		tmpl["Dataset"] = true
	...
	if err != nil {
		return Error(err, LoadErrorCode, "unable to load runsummaries sql template", "dbs.runsummaries.RunSummaries")
	}
	...
	if err != nil {
		return Error(err, QueryErrorCode, "unable to query run summaries", "dbs.runsummaries.RunSummaries")
	}
	return nil
}
